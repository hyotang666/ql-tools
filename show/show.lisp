(defpackage #:ql-tools.show(:use :cl)
  (:export
    #:show
    )
  )
...
;;;; SHOW
(defun show(name)
  (format t "~{~&~:(~A~) : ~A~}"(system-properties name)))
...
(defun system-properties(name)
  (let((system(find-system name)))
	...
	:maintainer ,(maintainer system)
	:archive-size ,(size system)
	:depends-on ,(depends-on system)
	:description ,(description system)
	:homepage ,(homepage system)))))

(defun find-system(name)
  (or (ql-dist:find-system name)
      (asdf:find-system name nil)))

(defmethod ql-dist:name((system asdf:system))
...
(defun size(system)
  (ql-dist:archive-size(ql-dist:release system)))

(defmethod ql-dist:release((system asdf:system))
  system)
(defmethod ql-dist:archive-size((system asdf:system))
  "Not archived")
...
(defun author(system)
  (with-call-asdf-accessor system #'asdf:system-author))

(defun licence(system)
  (with-call-asdf-accessor system #'asdf:system-licence))
